...
class OutboundOrder(db.Model):
    ...
    lines = db.relationship("OutboundOrderLine", backref="order", cascade="all, delete-orphan")
...
class OutboundOrderLine(db.Model):
    __tablename__ = "outbound_order_line"

    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    outbound_order_id = db.Column(db.BigInteger, db.ForeignKey("outbound_order.id"), nullable=False)
